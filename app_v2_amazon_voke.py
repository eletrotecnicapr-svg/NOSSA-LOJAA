...
.menu-toggle{display:none;position:absolute;right:22px;top:21px;width:46px;height:46px;border:1px solid rgba(255,255,255,.15);
...
def link_amazon_para_carrinho(link, quantidade=1):
    ...
    url = str(link).strip()
    parsed = urlparse(url)
    if "amazon.com.br" not in parsed.netloc.lower():
        return None
...
def comprar(produto_id):
    ...
    link_carrinho = link_amazon_para_carrinho(produto.get("link_afiliado"))
    if link_carrinho:
        return redirect(link_carrinho)

    if produto.get("link_afiliado"):
