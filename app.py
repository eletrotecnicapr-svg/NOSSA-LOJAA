...
.menu-mobile-wrap{position:relative;width:100%}.menu-toggle-real{display:none;border:0;background:rgba(255,255,255,.14);color:#fff;border-radius:13px;padding:10px 14px;font-size:24px;line-height:1;font-weight:900;cursor:pointer;-webkit-tap-highlight-color:transparent}.menu-nav-real{display:flex;flex-wrap:wrap;gap:9px;align-items:center}.menu-nav-real a{color:#fff!important}.menu-aberto{display:flex!important}@media(max-width:700px){.menu-toggle-real{display:inline-flex;align-items:center;justify-content:center}.menu-nav-real{display:none!important;flex-direction:column;align-items:stretch;width:100%;margin-top:9px;padding:10px;background:rgba(17,24,39,.98);border-radius:16px;box-shadow:0 12px 25px rgba(0,0,0,.22)}.menu-nav-real a{width:100%;box-sizing:border-box;text-align:center}.menu-mobile-wrap{display:block}}</style>'''
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  select.addEventListener('change',atualizar); atualizar();
        ...
        if not request.path.startswith('/admin'):
            ...
            menu_html = f'''<div class="menu-mobile-wrap"><button type="button" class="menu-toggle-real" aria-expanded="false" aria-controls="menu-principal">☰</button><div id="menu-principal" class="menu-nav-real"><a href="/">Início</a><a href="/produtos">Produtos</a><a href="/carrinho" class="carrinho-link">🛒 Carrinho <span class="carrinho-badge">{carrinho_qtd}</span></a><a href="/login" class="botao-admin">🔐 Área administrativa</a></div></div>'''
            ...
            js = r'''<script>
document.addEventListener('DOMContentLoaded',function(){
  const menuBtn=document.querySelector('.menu-toggle-real'); const menu=document.getElementById('menu-principal');
  if(menuBtn && menu){ menuBtn.addEventListener('click',function(){ const aberto=menu.classList.toggle('menu-aberto'); menuBtn.setAttribute('aria-expanded',String(aberto)); menuBtn.textContent=aberto?'✕':'☰'; }); }
  document.querySelectorAll('.produto').forEach(function(card){
    ...
    if(t&&b){ b.textContent='🛒 Adicionar ao carrinho'; b.href='/carrinho/adicionar?nome='+encodeURIComponent(t.textContent.trim()); b.removeAttribute('target'); b.removeAttribute('rel'); }
        ...
        if request.path == '/admin' and '/admin/senha' not in html_pagina and '</nav>' in html_pagina:
            html_pagina = html_pagina.replace('</nav>', '<a href="/admin/senha">🔑 Alterar senha</a><a href="/admin/como-vender">🧭 Como vender</a></nav>', 1)
...
def checkout():
    ...
    if request.method=='POST':
        ...
        if not email or '@' not in email or '.' not in email.split('@')[-1]:
            ...
            try:
                ...
                session['ultimo_mp_total']=sum(float(i.get('preco') or 0)*int(i.get('quantidade') or 1) for i in itens)
    ...
    return f'''<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Finalizar compra - Nossa Loja</title>{CARRINHO_STYLE}</head><body><main class="container"><div class="pagamento-box"><h1>💳 Finalizar compra</h1><p>Estes são os produtos próprios que serão cobrados pela Nossa Loja.</p>{aviso}{erro_html}<ul>{resumo}</ul><h2>Total: R$ {total:,.2f}</h2><form class="checkout-form" method="post"><label>E-mail do comprador</label><input type="email" name="email" value="{html.escape(email)}" placeholder="voce@email.com" required><button class="cart-btn checkout-own" type="submit">🔒 Pagar com Mercado Pago</button></form><p class="checkout-note">Você será levado ao ambiente seguro do Mercado Pago. Os dados do cartão não passam pela Nossa Loja.</p><a class="cart-btn secondary" href="/carrinho">← Voltar ao carrinho</a></div></main></body></html>'''
