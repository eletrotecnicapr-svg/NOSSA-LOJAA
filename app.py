...
def aplicar_estilos_e_componentes(response):
    ...
    try:
        ...
        if not request.path.startswith('/admin'):
            ...
            # Menu mobile sem depender de JavaScript: <details> funciona mesmo
            # quando o navegador bloqueia scripts ou o JS da página falha.
            menu_html = f'''<details class="menu-shell"><summary class="menu-toggle" aria-label="Abrir menu">☰</summary><div class="menu-nav"><a href="/">Início</a><a href="/produtos">Produtos</a><a href="/carrinho" class="carrinho-link">🛒 Carrinho <span class="carrinho-badge">{carrinho_qtd}</span></a><a href="/login" class="botao-admin">🔐 Área administrativa</a></div></details>'''
            ...
            if request.path in ('/','/produtos'):
                js=r'''<script>document.addEventListener('DOMContentLoaded',function(){document.querySelectorAll('.produto').forEach(function(card){var t=card.querySelector('h3'),b=card.querySelector('.botao');if(t&&b){b.textContent='🛒 Adicionar ao carrinho';b.href='/carrinho/adicionar?nome='+encodeURIComponent(t.textContent.trim());b.removeAttribute('target');b.removeAttribute('rel');}});});</script>'''
...
def carrinho():
    ...
    if itens:
        permitidos = itens_permitidos_pagamento()
        checkout_link = '<a class="cart-btn" href="/checkout">💳 Finalizar compra</a>' if permitidos else ''
        aviso_pagamento = ''
        if not permitidos and itens:
            aviso_pagamento = '<p class="checkout-note" style="width:100%;margin:0 0 8px">Os produtos Amazon são comprados na própria Amazon. O Mercado Pago fica reservado para produtos vendidos pela Nossa Loja.</p>'
        pagina+=f'<div class="cart-total"><div><span>Total</span><br><strong>R$ {total:,.2f}</strong></div><div style="width:100%;display:flex;flex-wrap:wrap;gap:9px;align-items:center">{aviso_pagamento}{checkout_link}{amazon}<a class="cart-btn secondary" href="/produtos">+ Continuar comprando</a><a class="cart-btn danger" href="/carrinho/limpar">Limpar carrinho</a></div></div>'
...
def checkout():
    ...
    if not itens:
        ...
        if todos and not MERCADOPAGO_PERMITIR_AFILIADOS:
            return '''<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Pagamento - Nossa Loja</title>''' + CARRINHO_STYLE + '''</head><body><main class="container"><div class="pagamento-box"><h2>💳 Pagamento</h2><p>Seu carrinho contém somente produtos de afiliado da Amazon.</p><p>Por segurança, a Nossa Loja não cobra esses produtos pelo Mercado Pago. O pagamento deve ser concluído na Amazon.</p><a class="cart-btn" href="/carrinho">← Voltar ao carrinho</a></div></main></body></html>'''
