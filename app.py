...
def aplicar_estilos_e_componentes(response):
    ...
    try:
        ...
        if not request.path.startswith('/admin'):
            ...
            menu_html = f'''<button type="button" class="menu-toggle" aria-label="Abrir menu" aria-expanded="false" onclick="document.querySelector('.menu-nav').classList.toggle('aberto')">☰</button><div class="menu-nav"><a href="/">Início</a><a href="/produtos">Produtos</a><a href="/carrinho" class="carrinho-link">🛒 Carrinho <span class="carrinho-badge">{carrinho_qtd}</span></a><a href="/login" class="botao-admin">🔐 Área administrativa</a></div>'''
            ...
            if request.path in ('/','/produtos'):
                js=r'''<script>document.addEventListener('DOMContentLoaded',function(){var toggle=document.querySelector('.menu-toggle');var menu=document.querySelector('.menu-nav');if(toggle&&menu){toggle.addEventListener('click',function(ev){ev.preventDefault();ev.stopPropagation();menu.classList.toggle('aberto');toggle.setAttribute('aria-expanded',menu.classList.contains('aberto')?'true':'false');});document.addEventListener('click',function(ev){if(!menu.contains(ev.target)&&ev.target!==toggle){menu.classList.remove('aberto');toggle.setAttribute('aria-expanded','false');}});}document.querySelectorAll('.produto').forEach(function(card){var t=card.querySelector('h3'),b=card.querySelector('.botao');if(t&&b){b.textContent='🛒 Adicionar ao carrinho';b.href='/carrinho/adicionar?nome='+encodeURIComponent(t.textContent.trim());b.removeAttribute('target');b.removeAttribute('rel');}});});</script>'''
        ...
        if request.path == '/admin' and '/admin/senha' not in html_pagina and '</nav>' in html_pagina:
            html_pagina=html_pagina.replace('</nav>','<a href="/admin/senha">🔑 Alterar senha</a></nav>',1)
...
def item_carrinho(p):
    return {'id':str(p.get('id')),'nome':p.get('nome','Produto'),'descricao':p.get('descricao') or '', 'preco':float(p.get('preco') or 0),'imagem_url':p.get('imagem_url') or '', 'link_afiliado':p.get('link_afiliado') or '', 'plataforma':p.get('plataforma') or 'Amazon','asin':p.get('asin') or '','quantidade':1}

@app.route('/carrinho')
def carrinho():
    itens=obter_carrinho(); total=sum(float(i.get('preco') or 0)*int(i.get('quantidade') or 0) for i in itens)
    partes=[]
    for i in itens:
        nome=html.escape(str(i.get('nome'))); q=int(i.get('quantidade') or 1); pid=quote(str(i.get('id')),safe=''); img=i.get('imagem_url')
        foto=f'<img src="{html.escape(str(img))}" alt="{nome}">' if img else '<div class="cart-sem-imagem">📦</div>'
        partes.append(f'<article class="cart-card">{foto}<div class="cart-info"><h3>{nome}</h3><p>Quantidade: {q}</p><div class="cart-actions"><div class="qty-control"><a href="/carrinho/menos?id={pid}">−</a><span>{q}</span><a href="/carrinho/mais?id={pid}">+</a></div><a class="cart-btn danger" href="/carrinho/remover?id={pid}">Remover</a></div></div></article>')
    ...
    for i in itens:
        ...
        if asin: itens_amazon.append((asin,int(i.get('quantidade') or 1)))
    ...
    amazon=f'<form method="POST" action="/amazon/adicionar-carrinho" style="display:inline-flex;flex-wrap:wrap;gap:8px"><input type="hidden" name="AssociateTag" value="pedro0ba1e-20">{campos_amazon}<button class="cart-btn" type="submit">🛒 Adicionar todos à Amazon</button></form>' if itens_amazon else ''
    pagina=f'''<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Carrinho — Nossa Loja</title><link rel="stylesheet" href="/static/style.css">{CARRINHO_STYLE}</head><body><header><div class="container"><h1>🛒 Nossa Loja</h1><nav></nav></div></header><main class="container"><div class="cart-page"><div class="hero"><h2>Seu carrinho</h2><p>Adicione vários produtos e altere as quantidades antes de continuar.</p></div>{corpo}'''
    if itens:
        checkout_link = ''
        if itens_permitidos_pagamento():
            checkout_link = '<a class="cart-btn" href="/checkout">💳 Finalizar compra</a>'
        pagina+=f'<div class="cart-total"><div><span>Total</span><br><strong>R$ {total:,.2f}</strong></div><div>{checkout_link}{amazon}<a class="cart-btn secondary" href="/produtos">+ Continuar comprando</a><a class="cart-btn danger" href="/carrinho/limpar">Limpar carrinho</a></div></div>'
...
def itens_permitidos_pagamento():
    '''Por segurança, afiliados não são cobrados pela Nossa Loja por padrão.'''
    itens = obter_carrinho()
    if MERCADOPAGO_PERMITIR_AFILIADOS:
        return itens
    return [i for i in itens if str(i.get('plataforma') or '').strip().lower() not in ('amazon','amazon afiliado','afiliado')]
...
def criar_order_mercadopago(itens, email=''):
    ...
    itens_mp=[]; total=Decimal('0')
    for i in itens:
        qtd=max(1,int(i.get('quantidade') or 1))
        preco=Decimal(str(i.get('preco') or 0)).quantize(Decimal('0.01'))
        if preco<=0: continue
        total += preco*qtd
        itens_mp.append({'title':str(i.get('nome') or 'Produto')[:120],'unit_price':f'{preco:.2f}','quantity':qtd,'unit_measure':'unit','total_amount':f'{(preco*qtd):.2f}'})
    if not itens_mp or total<=0:
        raise RuntimeError('O carrinho não possui produtos com preço válido para pagamento.')
    body={'type':'online','processing_mode':'manual','capture_mode':'automatic_async','total_amount':f'{total:.2f}','external_reference':f'nossa-loja-{uuid.uuid4().hex[:20]}','description':'Compra - Nossa Loja','items':itens_mp}
...
def checkout():
    ...
    if not itens:
        todos=obter_carrinho()
        if todos and not MERCADOPAGO_PERMITIR_AFILIADOS:
            return '''<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Pagamento - Nossa Loja</title>''' + CARRINHO_STYLE + '''</head><body><main class="container"><div class="pagamento-box"><h2>💳 Pagamento</h2><p>Os produtos atuais do carrinho são links de afiliado e não serão cobrados diretamente pela Nossa Loja.</p><p>Use o botão da Amazon para concluir a compra.</p><a class="cart-btn" href="/carrinho">← Voltar ao carrinho</a></div></main></body></html>'''
        return redirect(url_for('carrinho'))
    erro=''; email=''
    if request.method=='POST':
        email=texto_formulario('email')
        if email and ('@' not in email or '.' not in email.split('@')[-1]): erro='Digite um e-mail válido.'
        else:
            try:
                order=criar_order_mercadopago(itens,email); checkout_url=order.get('checkout_url') or ''
                if not checkout_url: raise RuntimeError('O Mercado Pago não devolveu a URL do checkout.')
                session['ultimo_mp_order']=order.get('id',''); session['ultimo_mp_total']=sum(float(i.get('preco') or 0)*int(i.get('quantidade') or 1) for i in itens); session.modified=True
    ...
    aviso='' if MERCADOPAGO_ACCESS_TOKEN else '<div class="pagamento-erro"><b>Pagamento ainda não ativado.</b><br>No Render, adicione a variável <code>MERCADOPAGO_ACCESS_TOKEN</code>.</div>'
    disabled='disabled' if not MERCADOPAGO_ACCESS_TOKEN else ''
    erro_html=f'<div class="pagamento-erro">{html.escape(erro)}</div>' if erro else ''
    return f'''<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Finalizar compra - Nossa Loja</title>{CARRINHO_STYLE}</head><body><main class="container"><div class="pagamento-box"><h1>💳 Finalizar compra</h1><p>Confira seu pedido antes de ir para o Mercado Pago.</p>{aviso}{erro_html}<ul>{resumo}</ul><h2>Total: R$ {total:,.2f}</h2><form class="checkout-form" method="post"><label>E-mail para o pagamento (opcional)</label><input type="email" name="email" value="{html.escape(email)}" placeholder="voce@email.com"><button class="cart-btn" type="submit" {disabled}>🔒 Continuar para o Mercado Pago</button></form><p class="checkout-note">O pagamento será processado no ambiente seguro do Mercado Pago. A Nossa Loja não armazena dados do cartão.</p><a class="cart-btn secondary" href="/carrinho">← Voltar ao carrinho</a></div></main></body></html>'''
