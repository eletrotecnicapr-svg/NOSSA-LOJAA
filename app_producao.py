...
MERCADOPAGO_AMBIENTE = os.environ.get("MERCADOPAGO_AMBIENTE", "production").strip().lower()
...
def validar_webhook_mp():
    """Valida x-signature do Mercado Pago. Em produção o segredo é obrigatório."""
    if not MERCADOPAGO_WEBHOOK_SECRET:
        return MERCADOPAGO_AMBIENTE != 'production'
...
def validar_credencial_mp():
    if not MERCADOPAGO_ACCESS_TOKEN:
        raise RuntimeError('Mercado Pago não configurado. Adicione MERCADOPAGO_ACCESS_TOKEN no Render.')
    if MERCADOPAGO_AMBIENTE == 'production' and MERCADOPAGO_ACCESS_TOKEN.startswith('TEST-'):
        raise RuntimeError('A Nossa Loja está em PRODUÇÃO, mas o Access Token configurado é de TESTE. Substitua pelo Access Token de produção.')
    if MERCADOPAGO_AMBIENTE not in ('production', 'test'):
        raise RuntimeError('MERCADOPAGO_AMBIENTE deve ser production ou test.')
...
def atualizar_pedido_por_order_mp(order):
    ...
    if status in ('processed','approved'):
        pagamento='Pago'; pedido_status='Pago — aguardando preparação'
    elif status in ('processing','created','action_required','pending'):
        pagamento='Aguardando pagamento'; pedido_status='Aguardando pagamento'
    elif status in ('failed','rejected'):
        pagamento='Recusado'; pedido_status='Pagamento recusado'
    elif status in ('canceled','expired'):
        pagamento='Cancelado'; pedido_status='Cancelado'
    elif status in ('refunded','charged_back'):
        pagamento='Reembolsado'; pedido_status='Reembolsado'
    else:
        pagamento='Aguardando pagamento'; pedido_status='Aguardando pagamento'
    dados={'status_pagamento':pagamento,'status_pedido':pedido_status}
...
def criar_order_mercadopago(itens, email=''):
    ...
    if MERCADOPAGO_WEBHOOK_SECRET or MERCADOPAGO_AMBIENTE == 'production':
...
def checkout():
    ...
    try:
        validar_credencial_mp()
        aviso=''
    except Exception as _mp_cfg:
        aviso=f'<div class="pagamento-erro"><b>Pagamento indisponível.</b><br>{html.escape(str(_mp_cfg))}</div>'
...
def pagamento_sucesso():
    ...
    order_id=session.get('ultimo_mp_order')
    order=buscar_order_mercadopago(order_id) if order_id else None
    if order:
        try:
            atualizar_pedido_por_order_mp(order)
        except Exception:
    ...
    if status in ('processed','approved'):
        ...
        texto='Seu pagamento foi confirmado pelo Mercado Pago. Seu pedido está aguardando preparação.'
    elif status in ('processing','created','action_required','pending'):
        titulo='⏳ Pagamento em processamento'
        texto='Recebemos seu pedido, mas o Mercado Pago ainda não confirmou o pagamento. Aguarde a atualização do status.'
    elif status in ('failed','rejected','canceled','expired'):
        titulo='❌ Pagamento não aprovado'
        texto='O Mercado Pago não confirmou este pagamento. Você pode voltar ao carrinho e tentar novamente.'
    else:
        titulo='🧾 Pedido recebido'
        texto='Seu pedido foi registrado. O status do pagamento será atualizado quando o Mercado Pago confirmar a transação.'
    numero=f'<p>Nº do pedido: <b>{html.escape(str(pedido_id))}</b></p>' if pedido_id else ''
    pagina = '<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Status do pagamento - Nossa Loja</title>' + CARRINHO_STYLE + '</head><body><main class="container"><div class="pagamento-ok"><h1>' + titulo + '</h1><p>' + html.escape(texto) + '</p>' + numero + '<a class="cart-btn" href="/">Voltar para a loja</a></div></main></body></html>'
    return pagina
